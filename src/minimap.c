...
void	image_pixel_put(t_game *game, int x, int y)
{
	...
	pixel = game->data.addr + (y * game->data.line_len + x * (game->data.bpp / 8));
...
void	minimap(t_game *game)
{
	...
	row = 0;
	game->square.size = MAP_SCALE * game->block;
...
// 	game->square.size = 200 * MAP_SCALE;
// 	game->square.x = game->player.x * MAP_SCALE;
// 	game->square.y = game->player.y * MAP_SCALE;
// 	draw_rect(game);
